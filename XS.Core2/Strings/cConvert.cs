...
namespace XS.Core2.Strings
{
    ...
    public class cConvert
    {
        ...
        /// 过滤非法字符，替换法
        /// </summary>
        /// <param name="strTemContent">内容</param>
        /// <param name="strTemTap">要替换的非法字符</param>
        /// <param name="strReplaceTap">要替换的内容</param>
        public static string ReplaceStr(string strTemContent, string strTemTap, string strReplaceTap, char charSplitTap)
        {
            string strReplaceContent = "";
            int iReplaceTap = 0;
            string[] sarrTemTap = SplitArray(strTemTap, charSplitTap);
            foreach (string strTap in sarrTemTap)
            {
                strReplaceContent = strTemContent.Replace(strTap, strReplaceTap);
                strTemContent = strReplaceContent;
                iReplaceTap = iReplaceTap + 1;

            }
            return strTemContent;
        }
        /// <summary>
        /// 替换多标签
        /// </summary>
        /// <param name="sOld">要替换的内容</param>
        /// <param name="sarrTemTap">标签数组</param>
        /// <param name="arrReplaceTap">标签数组对应的内容</param>
        /// <returns></returns>
        public static string ReplaceMore(string sOld, string[] sarrTemTap, string[] arrReplaceTap)
        {
            int iReplaceTap = 0;
            string strReplaceContent;
            foreach (string strTap in sarrTemTap)
            {
                strReplaceContent = sOld.Replace(strTap, arrReplaceTap[iReplaceTap]);
                sOld = strReplaceContent;
                iReplaceTap = iReplaceTap + 1;
...
我想html页面代码中的所有类似的字符串加上http://www.xrss.cn/，比如修改后为href="http://www.xrss.cn/css/css.css"     src="/XrssFile/2007-3/15/200731593339319.gif", 等
...
调用Repair方法即可：          
        ...
        private void Match(ref string documentText, ref int startIndex)
        {
            ...
            if (startIndex < documentText.Length - 1)//如果未到达末端
            {
                ...
                if (code.IndexOf("http://www.xrss.cn/") < 0)
                {
        ...
        /// <summary>
        /// 去掉字符串里的多余空格与回车换行,有多个空格只保留一个,循环次数多,适合少量字符
        /// </summary>
        /// <param name="strContent"></param>
        /// <returns></returns>
        public static string ClearSpace(string strContent)
        {
            ...
            for (int i = strNew.Length; i > 1; i = i - 1)
            {
                string strSpace = "";
                for (int j = 0; j <= i; j++)
                {
        ...
        /// <summary>
        /// 将一个字符串以某一特定字符分割成字符串数组,用多个字符来分隔：string[] sArray=str.Split(new char[2]{'j','s'}); 
        /// </summary>
        ...
        /// <summary>
        /// 将一个字符串以某一特定字符分割成字符串数组,用字符串分隔string[] sArray=Regex.Split(str,"js",RegexOptions.IgnoreCase);

        /// </summary>
        /// <param name="source">字符串</param>
        /// <param name="split">分割字符</param>
        /// <returns>string[]</returns>
        public static string[] SplitArrayOfStr(string source, string split)
        {
        ...
        {
            return GetString.AddLast(path.Replace("/", "\\"), "\\");
        }
        ...
        /// <returns></returns>
        public static string get_text_from_html(string strHTML)
        {
            Regex r = new Regex("<(.*?)>", RegexOptions.IgnoreCase | RegexOptions.Compiled);
            return r.Replace(strHTML, "");
        ...
        /// <summary>
        /// 格式化占用空间大小的输出 如 313523432432423 可能会是 3M
        /// </summary>
        ...
        /// <param name="strMark">分割符号,可以是"'|','.'"这种形式</param>
        /// <returns></returns>
        public static string[] stringsplit(string strText, char[] strMark)
        {

            string[] strArr = strText.Split(strMark);
        ...
        /// <summary>
        /// 获取一段汉字里的首个汉字的首写字母（大写）
        /// </summary>
        /// <param name="text"></param>
        /// <returns></returns>
        public static string ConvertSpellindexof(string text)
        {
            string strIndexof = string.Empty;
            strIndexof = ConvertSpellFirst(text).Substring(0, 1).ToUpper();
new string[]{"Dia","嗲"},
...
new string[]{"Duo","多朵夺舵剁垛跺惰堕掇哆驮度躲踱沲咄铎裰哚缍亸仛兊兌兑凙刴剟剫吋喥嚉嚲垜埵墮墯夛奪奲媠嫷尮崜嶞憜挅挆捶揣敓敚敠敪朶杕枤柁柂柮桗棰椯橢毲沰沱澤痥硾綞茤袳詑誃貀趓跢跥跿躱軃郸鄲鈬錞鍺鐸锗陀陊陏隋隓飿饳馱駄鬌鮵鵽點"},
...
new string[]{"Hua","话花化画华划滑哗猾豁铧桦骅砉侉劃劐吪哇嘩埖姡婲婳嫿嬅崋找搳摦撶敌杹椛槬樺檴浍澅澮獪璍畫畵硴磆稞粿糀繣腂舙芲華蒍蕐蘤蘳螖觟話誮諙諣譁譮輠釪釫鋘錵鏵驊魤鮭鲑鷨黊"},
...
new string[]{"Hwa","夻"},
new string[]{"I","乁"},
new string[]{"Ji","几及急既即机鸡积记级极计挤己季寄纪基激吉脊际汲肌嫉姬绩缉饥迹棘蓟技冀辑伎祭剂悸济籍寂期奇忌齐妓继集给革击圾箕讥畸稽疾墼洎鲚屐齑戟鲫嵇矶稷戢虮诘笈暨笄剞叽蒺跻嵴掎跽霁唧畿荠瘠玑羁丌偈芨佶赍楫髻咭蕺觊麂骥殛岌亟犄乩芰哜丮乁亼伋倚偮僟兾刉刏剤劑勣卙卟卽厝叝吇呰喞嗘嘰嚌坖垍堲塉墍妀姞姼尐居屰岋峜嵆嶯帺幾庴廭彐彑彶徛忣惎愱憿懠懻揖揤撃撠撽擊擠攲敧旡旣暩曁枅梞棋楖極槉槣樭機橶檕檝檵櫅櫭毄汥泲洁淁済湒漃漈潗濈濟瀱焏犱狤猗璂璣璾畟疵痵瘵癠癪皀皍睽瞉瞿磯禝禨秸稘稩穄穊積穖穧筓箿簊簎粢糭紀紒級結給継緝縘績繋繫繼结罽羇羈耤耭胔脔脨膌臮艥艻芶苙茍茤莋萁萕葪蒩蔇蕀蕲薊薺藉蘄蘎蘮蘻虀蜡蝍螏蟣蟻蟿蠀裚褀襀襋覉覊覘覬覿觇觌觙觭計訐記誋諅諔譏譤讦谻谿賫賷趌趞跂跡踑踖踦蹐蹟躋躤躸輯轚郅郆鄿銈銡錤鍓鏶鐖鑇鑙隔際隮雞雦雧霵霽鞊鞿韲颳飢饑騎驥骑鬾魝魢魥鮆鯚鯯鯽鰶鰿鱀鱭鱾鳮鵋鶏鶺鷄鷑鸄鹡齊齌齍齎齏"},
...
new string[]{"Le","了勒肋鳓仂叻泐嘞忇扐楽樂氻牞玏砳竻簕艻阞韷餎饹鰳"},
...
new string[]{"Luo","落罗锣裸骡烙箩螺萝洛骆逻络咯荦漯蠃雒倮硌椤捋脶瘰摞泺珞镙猡儸儽剆啰囉峈挼捰攎攞攭曪果格樂橐櫟欏欙濼烁爍犖猓玀癳皪砢硦碌礫笿籮絡纙羅腡臝茖蓏蘿蛒蜾蝸蠡袼覶覼詻跞路躒躶邏鉻鎯鏍鑼铬頱饠駱騾驘鮥鱳鵅鸁"},
...
new string[]{"Meng","猛梦蒙锰孟盟檬萌氓礞蜢勐懵甍蠓虻朦艋艨瞢儚冡夢夣嫇尨幪庬懜懞掹擝曚橗氋溕濛獴瓾甿瞑矇矒罞莔萠蕄蝱蟊蟒鄳鄸鋂錳雺霚霥霧霿靀顭饛髳鯍鯭鱦鸏鹲黽黾鼆"},
