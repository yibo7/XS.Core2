...
namespace XS.Core2
{
    ...
    public class IniParser
    {
        ...
        public IniParser(String iniPath, bool TextConfig)
        {
            Init(iniPath, TextConfig);
        }
        /// <summary>
        /// Opens the INI file at the given path and enumerates the values in the IniParser.
        /// </summary>
        /// <param name="iniPath">Full path to INI file.</param>
        private void Init(String iniPath,bool TextConfig)
        {
            ...
            if (string.IsNullOrEmpty(iniPath)) //不存在就为空，纯写入用的
                return;
        ...
        /// Save settings back to ini file.
        /// </summary>
        public void SaveSettings(Encoding encoding)
        {
            SaveSettings(IniFilePath, encoding);
        }
        ...
        /// </summary>
        /// <param name="newFilePath">New file path.</param>
        public void SaveSettings(String newFilePath, Encoding encoding)
        {
            if (newFilePath == null)
                throw new Exception("在保存Ini文件时，发现文件路径为空");
            ...
            try
            {
                ...
                TextWriter tw = new StreamWriter(newFilePath, false, encoding);
                tw.Write(iniContent);
                tw.Close();
            }
            catch (Exception ex)
        ...
        public void AddSetting(String sectionName, String settingName, object settingValue)
        {
            if (!Equals(settingValue, null))
                AddSetting(sectionName, settingName, settingValue.ToString());
        }


        public int GetSettingInt(string sectionName,string settingName)
        {
            return int.Parse(GetSetting(sectionName,settingName));

        }
        public long GetSettingLong(string sectionName, string settingName)
        {
            return long.Parse(GetSetting(sectionName,settingName));

        }
        public decimal GetSettingDecimal(string sectionName, string settingName)
        {
            return decimal.Parse(GetSetting(sectionName,settingName));

        }
        public DateTime GetSettingDateTime(string sectionName, string settingName)
        {
            return DateTime.Parse(GetSetting(sectionName,settingName));
